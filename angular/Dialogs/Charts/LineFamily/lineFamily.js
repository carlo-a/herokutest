/* Line Family MODULE */
app.controller('LineFamilyController', function ($mdSidenav, $scope, $mdDialog, callstate, DataFactory, $filter, svg, toms, TabularFactory) {
    //Variables
    ...
    $scope.chartTypes = [{ name: "Line", value: 'line' }, { name: "Spline", value: 'spline' }, { name: "Area", value: 'area' }, { name: "Areaspline", value: 'areaspline' }]
    ...
    */
    ...
    $scope.GenerateHighchart = function () {
        if ($scope.tabularInfo.rows.length > 0 && $scope.tabularInfo.columns.length > 0) {
            var series = TabularFactory.LineFamily.GenerateSeries($scope.originalData, $scope.tabularInfo);
            $scope.graphdata = TabularFactory.LineFamily.GenerateChart($scope.chartInfo, $scope.tabularInfo, series);
            $scope.ValidateStep(true);
        }
    }
...
app.controller('UpdateLineFamilyController', function ($mdSidenav, $scope, $mdDialog, callstate, DataFactory, $filter, svg, toms, TabularFactory, modalData) {
    //Variables
    $scope.SVG = svg;// .SVG;               //getting svg items from config
    $scope.callstate = callstate;           //create state or update state of modal
    $scope.modalTitle = "";                 //determines Create or Update
